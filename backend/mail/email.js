...
export const sendVerificationEmail = async (email, name, verificationToken) => {
    ...
    return sendEmail({ to: email, subject: "Verify Your Email", html: emailContent });
};

// Send welcome email
export const sendWelcomeEmail = async (email, name) => {
    const emailContent = generateWelcomeEmail(name);
    return sendEmail({ to: email, subject: "Welcome to DocPortal", html: emailContent });
};

// Send password reset email
export const sendPasswordResetEmail = async (email, resetURL) => {
    const emailContent = generatePasswordResetRequestEmail(resetURL);
    return sendEmail({ to: email, subject: "Reset Your Password", html: emailContent });
};

// Send reset success email
export const sendResetSuccessEmail = async (email) => {
    const emailContent = generatePasswordResetSuccessEmail();
    return sendEmail({ to: email, subject: "Password Reset Successful", html: emailContent });
};
